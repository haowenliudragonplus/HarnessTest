...
public class ReferenceFinderData
{
    ...
    public bool ReadFromCache()
    {
        ...
        //反序列化数据
        var fs = File.OpenRead(CACHE_PATH);
        try
        {
            var bf = new BinaryFormatter();
            var cacheVersion = (string) bf.Deserialize(fs);
    ...
    private void WriteToChache()
    {
        ...
        //序列化
        using (var fs = File.OpenWrite(CACHE_PATH))
        {
    ...
    {
        if (assetDict.TryGetValue(guid, out var ad) && ad.state != AssetState.NODATA)
        {
