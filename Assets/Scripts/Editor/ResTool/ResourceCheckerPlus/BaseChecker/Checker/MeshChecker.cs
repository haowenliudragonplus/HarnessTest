    ...
    public class MeshChecker : ObjectChecker
    {
        ...
        public class MeshDetail : ObjectDetail
        {
            public MeshDetail(Object obj, MeshChecker checker) : base(obj, checker)
            {
                ...
                checkMap.Add(checker.meshIsParticleEmitMesh, isParticleEmitMesh.ToString());
            }
        ...
        CheckItem meshIsParticleEmitMesh;
        ...
        public override void InitCheckItem()
        {
            ...
            meshIsParticleEmitMesh = new CheckItem(this, "粒子发射Mesh");
        ...
        private void AddMeshInternal<T>(GameObject rootObj) where T : Component
        {
            ...
            foreach(var v in coms)
            {
                ...
                AddObjectDetailByCheckModule(mesh, rootObj, v.gameObject, false);
            }
        }
        ...
        private void AddParticleSystemMeshInternal(GameObject rootObj)
        {
            ...
            foreach(var ps in psComs)
            {
                ...
                AddObjectDetailByCheckModule(mesh, rootObj, ps.gameObject, true);
            }
            //发射的Mesh
            ...
            foreach(var psRenderer in coms)
            {
                ...
                {
                    AddObjectDetailByCheckModule(meshArray[i], rootObj, psRenderer.gameObject , true);
                }
#else
                //5.3木有GetMesh方法...反射貌似也取不到，so...只取一个Mesh吧，一般Particle也只发射一个
                AddObjectDetailByCheckModule(psRenderer.mesh, rootObj, psRenderer.gameObject, true);
#endif
            }
        }

        private void AddObjectDetailByCheckModule(Object checkObj, Object rootObj, Object refObj, bool isParticleEmit)
        {
            ObjectDetail detail = null;
            //加速检查，修改一下AddObjectDetail使之直接返回detail，防止再遍历查找了
            if (checkModule is SceneResCheckModule)
                detail = AddObjectDetail(checkObj, refObj, null);
            else
                detail = AddObjectDetail(checkObj, rootObj, refObj);
            var meshDetail = detail as MeshDetail;
            if (meshDetail == null)
                return;
            meshDetail.isParticleEmitMesh |= isParticleEmit;
            meshDetail.checkMap[meshIsParticleEmitMesh] = meshDetail.isParticleEmitMesh.ToString();
        }
        ...
        public override void DirectResCheck(Object[] selection)
        {
            ...
            if (objects != null && objects.Count > 0)
            {
                for (int i = 0; i < objects.Count; i++)
                {
                    ...
                    Object[] dependency = EditorUtility.CollectDependencies(new Object[] { o });
