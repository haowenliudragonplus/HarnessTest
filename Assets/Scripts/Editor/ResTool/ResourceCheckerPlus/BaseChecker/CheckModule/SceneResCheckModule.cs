...
namespace ResourceCheckerPlus
{
    ...
    public class SceneResCheckModule : ResCheckModuleBase
    {
        ...
        {
            moduleType = CheckModule.SceneRes;
            checkModeName = new GUIContent("场景", "检查当前场景Hierarchy面板中全部或选中节点下包含的全部资源");
            CreateChecker(cfg);
        ...
        {
            if (GUILayout.Button("检查全场景资源", GUILayout.Width(ResourceCheckerPlus.instance.checkerConfig.sideBarWidth)))
            {
                CheckCurrentSceneTotalRes();
            }

            if (GUILayout.Button("检查Hierarchy选中节点下资源", GUILayout.Width(ResourceCheckerPlus.instance.checkerConfig.sideBarWidth)))
            {
                CheckResource(null);
            }
            EditorGUI.BeginChangeCheck();
            completeRefCheck = GUILayout.Toggle(completeRefCheck, completeRefCheckContent, GUILayout.Width(ResourceCheckerPlus.instance.checkerConfig.sideBarWidth));
