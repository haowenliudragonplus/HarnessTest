...
public class UISubView_GuideNPCText_Mid : UISubView_GuideNPCText_MidBase
{
    public void OnInit(Table_Common_Guide guideData)
    {
        if (guideData == null)
            return;
        //enable
        GO.SetActive(guideData.NPCTextEnable&& guideData.TextType=="2");
        if (!guideData.NPCTextEnable||
            guideData.TextType!="2") return;
        //text
