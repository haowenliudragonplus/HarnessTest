...
public class LoopScrollList : MonoBehaviour
{
    ...
    public Action<RectTransform> OnItemCreate;
    ...
    public void Init()
    {
        ...
        viewPort = scrollRect.GetComponent<RectTransform>().rect.size;
    ...
    public void ReBuild()
    {
        if (scrollRect == null || content == null || item == null)
            return;
        ResetChildren();

        Vector2 maskSize = scrollRect.GetComponent<RectTransform>().rect.size;
        int count = 0;
    ...
    private RectTransform CreateItem(int index)
    {
        ...
        else
        {
            ...
            {
                OnItemCreate(child as RectTransform);
            }
        }
    ...
    public void SetChildCount(int value, bool forceRender)
    {
        ...
        if (value > items.Count && items.Count < maxPerLine * totalCount)
        {
            ...
            for (int i = count; i < max; i++)
            {
                ...
                child.anchorMax = new Vector2(0,1);//都设置为左上角 初始化显示才正确
                child.anchorMin = new Vector2(0,1);
                child.pivot = new Vector2(0,1);
                if (arrangement == Arrangement.Vertical)
                    child.anchoredPosition3D = startPos +
                                          new Vector2(col * itemSize.x + (col) * columuSpace,
                                              -row * itemSize.y - (row) * rowSpace);
                else
                    child.anchoredPosition3D = startPos +
        ...
        if (arrangement == Arrangement.Horizontal)
        {
            float maxY = marginTop + marginBottom + itemSize.y * maxPerLine + rowSpace * (maxPerLine - 1);
            content.sizeDelta = new Vector2(marginLeft + marginRight + itemSize.x * rc + columuSpace * (rc - 1),
                Mathf.Max(viewPort.y,maxY));
        ...
        {
            float maxX = marginLeft + marginRight + itemSize.x * maxPerLine + columuSpace * (maxPerLine - 1);
            content.sizeDelta = new Vector2(Mathf.Max(maxX, viewPort.x),marginTop + marginBottom + itemSize.y * rc + rowSpace * (rc - 1));
    ...
    {
        get { return content.sizeDelta.x > viewPort.x + 0.01f; }
    }

    public bool VScrollingNeeded
    {
        get { return content.sizeDelta.y > viewPort.y + 0.01f; }
    }
